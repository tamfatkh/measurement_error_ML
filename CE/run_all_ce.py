#!/usr/bin/env python3
"""
run_all_CE.py: Generates classical measurement-error datasets and runs both regression and classification
Optuna-based pipelines on feature sets X, W1, W2 using multiple models. Saves results incrementally.
"""
# to test the code use: python3 -m py_compile run_all_ce.py
# to run the code use: python3 run_all_ce.py
...
# === 1) DATA GENERATION ===
def simulate_classical_error_data(n, a0, a1, a2, a3):
    X  = np.random.normal(size=n)
    W1 = X + np.random.normal(scale=0.5, size=n)
    W2 = X + np.random.normal(scale=0.75, size=n)
...
def generate_all_data(param_sets, n_simulations=10, outdir="CEdata"):
    os.makedirs(outdir, exist_ok=True)
    for idx, (n, a0, a1, a2, a3) in enumerate(param_sets, start=1):
        n = int(n)
        for i in range(1, n_simulations+1):
            X, W1, W2, Z1, Z2, Y, p, B = simulate_classical_error_data(n, a0, a1, a2, a3)
            ...
            df.to_csv(f"{outdir}/CE_data_with_split_{idx}_{i}.csv", index=False)
...
def run_optimization(task):
    ...
    df[['X','W1','W2','Z1','Z2','Y']] = scaler.fit_transform(
        df[['X','W1','W2','Z1','Z2','Y']])
    ...
    out_fn = f"CEdata/results_{idx}_{sim}.csv"
...
# === 6) TASK COLLECTION ===
def collect_tasks(datadir="CEdata"):
    tasks=[]
...
if __name__=="__main__":
    ...
    n_sim = 10
    if not os.path.isdir("CEdata") or len([f for f in os.listdir("CEdata") if f.startswith("CE_data_with_split_")])<len(param_sets)*n_sim:
        generate_all_data(param_sets,n_sim,"CEdata")
    tasks=collect_tasks("CEdata")
    ...
    # merge
    import glob
    files=glob.glob("CEdata/results_*.csv")
    pd.concat([pd.read_csv(f) for f in files],ignore_index=True).to_csv("CEdata/combined_results_R_and_C.csv",index=False)
