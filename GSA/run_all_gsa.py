#!/usr/bin/env python3
"""
run_all_gsa.py: Generates GSA measurement-error datasets and runs both regression and classification
Optuna-based pipelines on feature sets X, W1, W2 using multiple models. Saves results incrementally.
"""
# to test the code use: python3 -m py_compile run_all_gsa.py
# to run the code use: python3 run_all_gsa.py
...
# === 1) DATA GENERATION ===
def simulate_gsa_data(n, a0, a1, a2, a3):
    g = 5
    mu = np.random.normal(0, 1, size=g)
    W = np.random.choice(mu, size=n)
...
def generate_all_data(param_sets, n_simulations=10, outdir="GSAdata"):
    os.makedirs(outdir, exist_ok=True)
    for idx, (n, a0, a1, a2, a3) in enumerate(param_sets, start=1):
        n = int(n)
        for i in range(1, n_simulations+1):
            X1, X2, W, Z1, Z2, Y1, Y2, p1, B1, p2, B2 = simulate_gsa_data(n, a0, a1, a2, a3)
            ...
            df.to_csv(f"{outdir}/GSA_data_with_split_{idx}_{i}.csv", index=False)
...
def run_optimization(task):
    ...
    out_fn = f"GSAdata/results_{idx}_{sim}.csv"
...
# === 6) TASK COLLECTION ===
def collect_tasks(datadir="GSAdata"):
    tasks=[]
...
if __name__=="__main__":
    ...
    n_sim = 10
    if not os.path.isdir("GSAdata") or len([f for f in os.listdir("GSAdata") if f.startswith("GSA_data_with_split_")])<len(param_sets)*n_sim:
        generate_all_data(param_sets,n_sim,"GSAdata")
    tasks=collect_tasks("GSAdata")
    ...
    # merge
    import glob
    files=glob.glob("GSAdata/results_*.csv")
    pd.concat([pd.read_csv(f) for f in files],ignore_index=True).to_csv("GSAdata/combined_results_R_and_C.csv",index=False)
