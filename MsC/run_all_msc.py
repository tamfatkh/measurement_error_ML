#!/usr/bin/env python3
"""
run_all_msc.py: Generates misclassification measurement-error datasets and runs both regression and classification
Optuna-based pipelines on feature sets X, W1, W2 using multiple models. Saves results incrementally.
"""
# to test the code use: python3 -m py_compile run_all_msc.py
# to run the code use: python3 run_all_msc.py
...
# === 1) DATA GENERATION ===
def simulate_misclassification_data(n, a0, a1, a2, a3):
    X = np.random.binomial(1, 0.5, size=n)
    delta1 = np.random.binomial(1, .1, size=n)
    W1 = (1-delta1) * X + delta1 * (1-X)
    delta2 = np.random.binomial(1, .3, size=n)
    W2 = (1-delta2) * X + delta2 * (1-X)
    Z1 = np.random.normal(size=n)
    Z2 = np.random.normal(size=n)
    L  = a0 + a1*X + a2*Z1 + a3*Z2
    Y  = L + np.random.normal(scale=0.1, size=n)
    p  = 1/(1+np.exp(-L))
    B  = np.random.binomial(1, p, size=n)
    return X, W1, W2, Z1, Z2, Y, p, B


def generate_all_data(param_sets, n_simulations=10, outdir="MsCdata"):
    os.makedirs(outdir, exist_ok=True)
    for idx, (n, a0, a1, a2, a3) in enumerate(param_sets, start=1):
        n = int(n)
        for i in range(1, n_simulations+1):
            X, W1, W2, Z1, Z2, Y, p, B = simulate_misclassification_data(n, a0, a1, a2, a3)
            df = pd.DataFrame({"X":X, "W1":W1, "W2":W2, "Z1":Z1, "Z2":Z2, "Y":Y, "p":p, "B":B})
            train_idx, test_idx = train_test_split(df.index, test_size=0.2, stratify=df['B'], random_state=42)
            df['train_test_split'] = 0
            df.loc[train_idx, 'train_test_split'] = 1
            df.to_csv(f"{outdir}/MsC_data_with_split_{idx}_{i}.csv", index=False)
...
def run_optimization(task):
    ...
    df[['Z1','Z2','Y']] = scaler.fit_transform(
        df[['Z1','Z2','Y']])
    train = df[df.train_test_split == 1]
    test  = df[df.train_test_split == 0]

    # 2) Pull out features & targets
    Xtr_X, Xte_X = train[['X','Z1','Z2']], test[['X','Z1','Z2']]
    ytr_reg, yte_reg = train.Y, test.Y
    Xtr_W1, Xte_W1 = train[['W1','Z1','Z2']], test[['W1','Z1','Z2']]
    Xtr_W2, Xte_W2 = train[['W2','Z1','Z2']], test[['W2','Z1','Z2']]
    ytr_clf, yte_clf = train.B, test.B
    ...
    for m in clf_models:
        for label, Xtr, Xte in [("X",Xtr_X,Xte_X),("W1",Xtr_W1,Xte_W1),("W2",Xtr_W2,Xte_W2)]:
            ...
            study.optimize(lambda t: safe_obj(m, t, Xtr, Xte, ytr_clf, yte_clf),
                           n_trials=20, catch=(ValueError,), show_progress_bar=False)
            acc, f1, perm = train_eval_clf(m, study.best_params,
                                          Xtr, Xte, ytr_clf, yte_clf)
            results.append([idx, sim, label, m,
                            study.best_value, acc, f1,
                            json.dumps(study.best_params),
                            json.dumps(perm)])

    out_fn = f"MsCdata/results_{idx}_{sim}.csv"
...
# === 6) TASK COLLECTION ===
def collect_tasks(datadir="MsCdata"):
    tasks=[]
...
if __name__=="__main__":
    ...
    # param_sets=[(4000,0,2,1,0)]
    n_sim = 10
    if not os.path.isdir("MsCdata") or len([f for f in os.listdir("MsCdata") if f.startswith("MsC_data_with_split_")])<len(param_sets)*n_sim:
        generate_all_data(param_sets,n_sim,"MsCdata")
    tasks=collect_tasks("MsCdata")
    ...
    # merge
    import glob
    files=glob.glob("MsCdata/results_*.csv")
    pd.concat([pd.read_csv(f) for f in files],ignore_index=True).to_csv("MsCdata/combined_results_R_and_C.csv",index=False)
